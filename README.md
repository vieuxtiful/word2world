...
- Weighted fusion: α_text=0.6, α_image=0.2, α_network=0.2
...
```
x_i^(multimodal) = [α_text · x_i^(text), α_image · x_i^(image), α_network · x_i^(network)]
```
...
- `Σ` is the covariance matrix estimated from all user embeddings
...
class STAPv4Config:
    ...
    alpha_image: float = 0.2
    alpha_network: float = 0.2
